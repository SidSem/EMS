...
function ProductRow({ product, onEdit, onDelete, canEdit, canDelete }) {
  ...
  return (
    <tr className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors duration-150">
      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400">{productId}</td>
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700 dark:text-slate-300">{quantity}</td>
      {(canEdit || canDelete) && (
